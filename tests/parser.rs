...
fn test_two_newline() {
    ...
    assert_eq!(
        result,
        vec!(Instruction::EmptyLine, Instruction::EmptyLine,)
    );
}
...
"#;
    let result: Vec<Instruction> = parse_asm(text)
        .unwrap()
        .into_iter()
        .map(|x| x.unwrap())
        .collect();
...
"#;
    ...
    assert_eq!(
        ...
        vec!(
            ...
            Instruction::JpI16(
                Flag::Always,
                Expr::unary(Expr::Ident(String::from("foo")), UnaryOperator::Minus)
            ),
            Instruction::JpI16(
                Flag::Always,
                Expr::binary(
            ...
            Instruction::JpI16(
                Flag::Always,
                Expr::binary(Expr::Const(413), BinaryOperator::Mul, Expr::Const(1111))
            ),
            Instruction::JpI16(
                Flag::Always,
                Expr::binary(
        ...
        vec!(
            ...
            Instruction::JpI16(
                ...
                Expr::binary(
                    Expr::Ident(String::from("foo")),
                    BinaryOperator::Mul,
                    Expr::unary(Expr::Ident(String::from("bar")), UnaryOperator::Minus)
                )
            ),
            Instruction::JpI16(
                Flag::Always,
