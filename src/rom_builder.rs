...
impl RomBuilder {
    ...
    pub fn add_audio_data(mut self, file_name: &str, identifier: &str, finish_action: AudioFinishAction) -> Result<Self, Error> {
        ...
        let lines = audio::parse_audio_text(&text)?;
    ...
    /// ```asm
    /// GGBASMMusicEnable    EQU 0xC020 ; dont process music when 0 otherwise process it
    ...
    /// ```
    ///
    /// Change the currently playing song by setting GGBASMMusicBank, GGBASMMusicPointerHi and GGBASMMusicPointerLo to the
    ...
    pub fn add_audio_player(self) -> Result<Self, Error> {
        ...
        let instructions = parser::parse_asm(&text).unwrap().into_iter().map(|x| x.unwrap()).collect();
